...
fn part_1(input: &str) -> u32 {
    input.lines().fold(0, |acc, x| {
            let xs = x.split_whitespace()//.filter(|x| *x != ' ')
                      .map(|x| x.parse::<u32>().unwrap())
                      .collect::<Vec<u32>>();
            acc + xs.iter().max().unwrap() - xs.iter().min().unwrap()
        }
    )
}
...
fn part_2(input: &str) -> u32 {
    input.lines().fold(0, |acc, x| {
            let xs = x.split_whitespace()
                      .map(|x| x.parse::<u32>().unwrap())
                      .collect::<Vec<u32>>();
            acc + iproduct!(xs.iter(),xs.iter())
                    .filter(|x|{
                        x.0 > x.1 && x.0 != x.1 && (x.0 % x.1 == 0 || x.1 % x.0 == 0)
                    }).fold(0, |acc, x| {
                        acc + (x.0 / x.1)
                    })
        }
...
214	221	265	766	959	1038	226	1188	1122	117	458	1105	1285	1017	274	281";
    // assert!(part_1(test) == 18);
    assert!(part_2(test) == 9);
